...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const experienceTabButton = document.querySelector('#experience-tab'); // Update if needed

    // Show items with slide-in effect when tab is clicked
    if (experienceTabButton) {
        experienceTabButton.addEventListener('click', function () {
        ...
        // Auto-play if no tab button is used (like on page load)
        experienceItems.forEach((item, index) => {
            setTimeout(() => {
                item.classList.add('active');
            }, index * 300);
